...
.EXAMPLE
    .\shrinkwrap.ps1 -TargetSizeMB 8 -Preset faster
...
function Record-Summary {
    ...
    $Reduction = "N/A"
    if ($OrigSize -gt 0 -and $FinalSize -gt 0) {
        $Reduction = [math]::Round((($OrigSize - $FinalSize) / $OrigSize) * 100, 2)
    }
...
function Invoke-FFmpegEncode {
    ...
    # Video params
    $Args += "-c:v", $Script:VideoCodec
    $Args += "-pix_fmt", "yuv420p"
    
    if ($VideoParams.Bitrate) {
        $Args += "-b:v", "$($VideoParams.Bitrate)k"
    }
    if ($VideoParams.CRF) {
        $Args += "-crf", $VideoParams.CRF
    }
    if ($VideoParams.Preset) {
        $Args += "-preset", $VideoParams.Preset
    }
    if ($VideoParams.Scale) {
        $Args += "-vf", $VideoParams.Scale
    }
    
    # Audio params
    if ($Pass -eq 1) {
        $Args += "-an" # No audio in pass 1
    } else {
        $Args += "-c:a", "aac"
        $Args += "-b:a", "$($AudioParams.Bitrate)k"
        $Args += "-ac", "2"
    }
    ...
    # Execute
    $Process = Start-Process -FilePath $Script:FFmpeg -ArgumentList $Args `
        -NoNewWindow -Wait -PassThru -RedirectStandardError "$OutputFile.log"
...
function Invoke-RescueMode {
    ...
    while ($Retries -lt $MaxRetries) {
        ...
        $ExitCode2 = Invoke-FFmpegEncode -InputFile $InputFile -OutputFile $OutputFile `
            -VideoParams @{ Bitrate=$CurrentVideoKbps; Preset=$Preset; Scale="scale='min(1920,iw)':-2" } `
            -AudioParams @{ Bitrate=$MinAudioBitrate } -PassLogFile $PassLog -Pass 2
...
function Optimize-Video {
    ...
    while ($Retries -lt $MaxRetries) {
        ...
        $ExitCode2 = Invoke-FFmpegEncode -InputFile $InputFile -OutputFile $TempFile `
            -VideoParams @{ Bitrate=$CurrentVideoKbps; Preset=$Preset; Scale="scale='min(1920,iw)':-2" } `
            -AudioParams @{ Bitrate=$AudioBitrateKbps } -PassLogFile $PassLog -Pass 2
        Show-Progress "Pass 2" "Encoding" 100
        Write-Host ""
...
Write-ColorOutput "Configuration: Codec=$($Script:VideoCodec) | Vsync=$($Script:VsyncFlag)" "Gray"
